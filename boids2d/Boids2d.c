...
void Flock_bang(FlockPtr flockPtr)
{
	...
	switch(flockPtr->mode) { // newpos
		...
		//	SETFLOAT(out+3, flockPtr->boid[i].newPos.z);
			outlet_list(flockPtr->out1, 0L, 3, out);
		}
		break;
		...
		for (i = 0; i < flockPtr->numBoids; i++){						
			...
			azi = atan2(delta_y, delta_x) * flockPtr->r2d;
...
void Flock_dump(FlockPtr flockPtr)
{
	...
	outlet_anything(flockPtr->out2, gensym("flyrect"), 4, outList);
	...
	outlet_anything(flockPtr->out2, gensym("attractpt"), 2, outList);
